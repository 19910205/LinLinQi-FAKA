...
type api14Protocol struct{ unsupportedShopcloneProtocol }

func init() { registerShopcloneProtocol("api-14", api14Protocol{}) }

func api14Authorization(gateway *shopcloneGateway) http.Header {
	return http.Header{"Authorization": {gateway.credentials["token"]}}
}

func (api14Protocol) catalog(ctx context.Context, gateway *shopcloneGateway) ([]Category, []Product, error) {
	response, err := gateway.postFormObject(ctx, "/api", url.Values{"act": {"Get-Products"}}, api14Authorization(gateway))
	if err != nil {
		return nil, nil, err
	}
...
func (api14Protocol) balance(ctx context.Context, gateway *shopcloneGateway) (protocolBalance, error) {
	response, err := gateway.postFormObject(ctx, "/api", url.Values{"act": {"Me"}}, api14Authorization(gateway))
	if err != nil {
...
func (api14Protocol) createOrder(ctx context.Context, gateway *shopcloneGateway, input CreateOrderRequest) (OrderResult, error) {
...
func (api14Protocol) order(ctx context.Context, gateway *shopcloneGateway, externalNo string) (OrderResult, error) {
	response, err := gateway.postJSONObject(ctx, "/api", nil, map[string]any{
		"act":  "Get-Order",
